...
export const StatusBar = memo(function StatusBar({ connected, snapshot, onToggleSidebar }: Props) {
  ...
  return (
    <header className="h-12 bg-anima-card border-b border-anima-border flex items-center px-4 gap-4 flex-shrink-0">
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs text-anima-muted hidden sm:inline">员工控制中心</span>
      </div>
      ...
      {/* 状态指标 */}
      <div className="flex items-center gap-2 sm:gap-4 text-xs text-anima-muted">
        <span className="hidden md:inline">模型: <span className="text-anima-accent">{model}</span></span>
        <span>信任: <span className="text-anima-warm">{snapshot?.trust?.score ?? 0}分</span></span>
        <span className="hidden sm:inline">技能: <span className="text-anima-accent2">{snapshot?.skills?.length ?? 0}个</span></span>
